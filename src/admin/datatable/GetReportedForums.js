...
export const GetReportedForums = () => {
    ...
    const columns = [
        ...
        },
        {
            name: "Action",
            width: "150px",
            cell: (row) => <button className="btn btn-danger mx-auto pl-1 btn-sm w-100" onClick={() => deleteForumFunc(row.forum_id)}>Delete forum</button>,
            button: true,
            allowOverflow: true
        }
    ]
    ...
    const openReportedByModal = async (forum_id) => {
        dispatch(togglePRBModal({
            visible: true,
            data: {
                confession_id: forum_id
            }
        }))
    }

    const deleteForumFunc = async (forum_id) => {

        let confirm = window.confirm("Are you sure, you want to delete this forum?")
        if (!confirm) return
    ...
    return (
        <div>
            ...
            {postRBYModalReducer.visible &&
                <PostReportedByModal
                    title="Forum Reported By"
                    endPoint="getreportedforumusers"
                    isUsedForForums={true} />}
